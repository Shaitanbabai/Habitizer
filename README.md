...
3. модуль database
4. модуль statistics
5. база данных habit_tracker.db

1. Модуль config.py предназначен для хранения и управления конфигурационными параметрами приложения.

	1. Параметры:

	- TELEGRAM_API_TOKEN: Токен для доступа к Telegram API. Его можно получить у @BotFather в Telegram.
	Значение по умолчанию: your-telegram-api-token-here.
	Можно указать в переменной окружения TELEGRAM_API_TOKEN.
	- DB_NAME: Название файла базы данных SQLite.
	Значение по умолчанию: habit_tracker.db.
	Можно указать в переменной окружения DB_NAME.
	- DB_PATH: Путь к файлу базы данных.
	Значение по умолчанию: текущая директория.
	Можно указать в переменной окружения DB_PATH.

	2. Функции:

	get_db_uri(): Возвращает полный путь к файлу базы данных.

	Использование переменных окружения:
	Для более безопасного хранения конфиденциальных данных, таких как токен API, рекомендуется использовать переменные окружения.

	# Загрузка переменных окружения из .env файла
	load_dotenv()

Модуль config.py предоставляет удобный способ хранения и управления конфигурационными параметрами приложения,
а также поддерживает использование переменных окружения для более безопасного хранения конфиденциальных данных.


2. Модуль main.py является основной частью бота-трекера привычек. Он содержит функции для:

	1. Инициализации бота:

	Подключается к API Telegram с помощью библиотеки telebot.
	Определяет константы, необходимые для работы бота.
	Инициализирует экземпляр класса HabitTrackerDatabase для работы с базой данных.

	2. Обработки команд:

	Определяет функции-обработчики для команд /start, /help, /habit, /change, /delete, /statistics.
	Команда /start приветствует пользователя, регистрирует его в базе данных (если он не зарегистрирован) и запускает фоновый поток для отправки напоминаний.
	Команда /help выводит список доступных команд.
	Команда /habit позволяет пользователю добавить новую привычку в базу данных.
	Команды /change и /delete пока находятся в разработке.
	Команда /statistics пока находится в разработке.

	3. Обработки текстовых сообщений:
	Пока не реализовано.

	4. Обработки поиска:
	Пока не реализовано.

	5. Отправки напоминаний:
	Фоновый поток check_and_send_reminders регулярно проверяет время и отправляет напоминания о выполнении привычек.
	Напоминания отправляются в виде сообщений с inline-кнопкой "Отметить выполнение".

	6. Обработки нажатий на inline-кнопки:
	Функция handle_complete_habit обрабатывает нажатие на кнопку "Отметить выполнение".
	Она обновляет статус привычки в базе данных и отправляет пользователю сообщение о том, что его привычка отмечена как выполненная.

	7. Логирования:
	Функция send_reminder_and_log_statistics записывает лог отправки напоминания в таблицу statistics базы данных.

В целом модуль main.py обеспечивает основную функциональность бота-трекера привычек, позволяя пользователям добавлять, отслеживать и выполнять свои привычки.

3. Модуль database.py предназначен для работы с базой данных SQLite, которая используется для хранения информации о пользователях, их привычках и напоминаниях.

	1. Основные функции:

	- Создание соединения с базой данных:
    	* `create_connection(db_file)` - инициализирует соединение с базой данных, указанной в `db_file`.
	
	- Работа с пользователями:
    	* `add_user(user_tg_id, user_name)` - добавляет нового пользователя в базу данных.

	- Работа с привычками:
	* `add_habit(user_id, habit_name, habit_description, habit_frequency, reminder_time_from, reminder_time_till)` - добавляет новую привычку для пользователя.
	* `is_habit_duplicate(user_id, habit_name, habit_description)` - проверяет, является ли привычка дубликатом.
	* `get_habit_id(user_id, habit_name, habit_description)` - получает ID привычки.
	
	- Работа с напоминаниями:
	* `get_due_reminders(current_time)` - получает напоминания, которые должны быть отправлены в текущее время.
	* `send_reminder_and_log_statistics(user_id, habit_id, reminder_id)` - записывает событие отправки напоминания пользователю в таблицу статистики.
	* `add_user_response(user_id, habit_id, reminder_id, response_time)` - записывает отклик пользователя на напоминание в таблицу статистики.

	2. Структура базы данных:

	База данных состоит из 4 таблиц:

	* `users`: Содержит информацию о пользователях, включая ID пользователя, ID Telegram, имя пользователя, статус пользователя, дату регистрации и уровень доступа.
	* `habits`: Содержит информацию о привычках, включая ID привычки, ID пользователя, название привычки, описание привычки, частоту выполнения, статус привычки, дату начала привычки, дату окончания привычки, удобное время начала оповещения и удобное время окончания оповещения.
	* `reminders`: Содержит информацию о напоминаниях, включая ID напоминания, ID пользователя, ID привычки, время напоминания и статус напоминания.
	* `statistics`: Содержит статистику выполнения привычек, включая ID пользователя, ID привычки, ID напоминания, статус выполнения привычки, статус выполнения напоминания, название статуса выполнения напоминания, время отправки напоминания, время получения ответа пользователя на напоминание.

Модуль `database` использован для создания, чтения, обновления и удаления информации о пользователях, их привычках и напоминаниях. Он также может быть использован для получения статистики выполнения привычек.

4. Модуль `statistics` используется для анализа и визуализации данных о выполнении привычек в приложении Habit Tracker. Он позволяет:

	1. Запрашивать данные:

	* Количество пропущенных напоминаний (`missed_number_reminders_habit`)
	* Количество выполненных напоминаний (`completed_number_reminders_habit`)
	* Количество отправленных напоминаний (`habit_status`)
	* Количество выполненных напоминаний за период (`reminders_status`)

	2. Строить диаграммы:

	* Круговая диаграмма: показывает процентное соотношение выполнения и невыполнения привычки за весь период и за интересующий период.
	* Сравнительная гистограмма: сравнивает процент невыполненных напоминаний за весь период и за интересующий период.

	3. Обрабатывать ошибки:

	* Модуль отлавливает ошибки, возникающие при работе с базой данных SQLite, и сообщает о них пользователю.
	* Обрабатывает ошибки деления на ноль, которые могут возникнуть при расчете процентов.

	4. Взаимодействие с пользователем:

	* Запрашивает у пользователя имя пользователя, имя привычки и даты для анализа за период.
	* Выводит построенные диаграммы на экран.

	5. Используемые функции:

	* `execute_query` - функция из модуля `database`, которая используется для выполнения SQL-запросов к базе данных.
	* `matplotlib.pyplot` - библиотека для создания диаграмм в Python.
	* `datetime` - модуль для работы с датами и временем.

	6. Дополнительные возможности:

	* Модуль можно расширить, добавив функции для:
	    * Сохранения построенных диаграмм в виде файлов.
	    * Отправки диаграмм по электронной почте.
	    * Анализа выполнения привычек за более длительные периоды.

Модуль `statistics` является полезным инструментом для анализа и визуализации данных о выполнении привычек в приложении Habit Tracker. Он позволяет пользователям отслеживать свой прогресс и принимать решения о том, как улучшить свои привычки.
